...
class QueryBase(abc.ABC):
    ...
    def params(self) -> dict:
        return {self._request_type_field: self._request_type, **self._params}
    ...
    def validate_params(self, params: dict or None) -> None:
        ...
        if self._required_params:
            ...
            if omitted_params:
                raise ClientException(f"Omitted required params: {omitted_params}")

        unknown_params = params_keys - (self._required_params | self._optional_params)
        if unknown_params:
            raise ClientException(f"Unknown params: {unknown_params}")
...
class GetPeers(QueryBase):
    ...
    _response_json_schema = {
        ...
        "required": ["peers", "requestProcessingTime"],
    }
...
class GetUnconfirmedTransactions(QueryBase):
    ...
    _response_json_schema = {
        ...
        "required": ["unconfirmedTransactions", "requestProcessingTime"],
    }
    # TODO: validate each
