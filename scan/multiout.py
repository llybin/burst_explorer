...
def set_aggr_block_signature(service: str, height: int, block_signature: bytes) -> None:
    key = 'aggr:{}:height:{}'.format(service, height)
...
def find_last_actual_aggr_block(height: int) -> int:
    heights = MultiOut.objects.values_list(
        'height', flat=True
    ).filter(height__lte=height).order_by('-height').distinct()

    for h in heights.iterator(16):
        bs0 = Block.objects.using('java_wallet').values_list(
            'block_signature', flat=True
        ).filter(height=h).first()
...
def clean_out_all() -> None:
    ...
    cursor.execute('TRUNCATE TABLE {}'.format(MultiOut._meta.db_table))
...
def aggregate_cmd():
    ...
    last_aggr_height = MultiOut.objects.values_list(
        'height', flat=True
    ).order_by('-height').first()

    if last_aggr_height:
        last_aggr_block_signature = get_aggr_block_signature('multiout', last_aggr_height)

        if last_block['height'] == last_aggr_height and last_block['block_signature'] == last_aggr_block_signature:
            ...
            if is_not_forked:
                ...
                _block_signature = Block.objects.using('java_wallet').values_list(
                    'block_signature', flat=True).get(height=last_aggr_height)
            ...
            if is_not_forked:
                ...
                height = find_last_actual_aggr_block(min(last_block['height'], last_aggr_height))
